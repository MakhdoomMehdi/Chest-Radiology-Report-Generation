...
OUTPUT:
    <OUT_DIR>/
        ...
        train.json       ← list of {subject_id, study_id, image, report}
...
def extract_study_records(csv_path: Path, split_name: str, max_samples: int = None) -> list:
    """
    Read a subject-level MIMIC-CXR CSV and return a flat list of study records.
    Each record is a dict: {subject_id, study_id, src (abs path), dst (rel output path), report}.
    """
    records = []
    skipped = {
    ...
    total_seen = len(records) + sum(skipped.values())
    print(f"\n[{split_name}]")
...
train_records = extract_study_records(
    DATASET_ROOT / "mimic_cxr_aug_train.csv", "train", MAX_TRAIN_SAMPLES
)
val_records = extract_study_records(
    DATASET_ROOT / "mimic_cxr_aug_validate.csv", "val", MAX_VAL_SAMPLES
)
...
# Each record: {subject_id, study_id, image (relative path), report (text)}.
...
print("  then open xray_researcher.ipynb and run from Step 1 onwards.")
print("═"*60)
